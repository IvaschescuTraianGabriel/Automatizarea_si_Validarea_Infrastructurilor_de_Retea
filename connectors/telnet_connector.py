...
class TelnetConnector:
    ...
    def configure_ftd(self):
        ...
        if any(k in out for k in ['Press <ENTER>', 'You must change', '--More--']):
            ...
            if 'Press <ENTER>' in out:
                self.write('\n')
                time.sleep(1)
                out = self.read()

            # Handle '--More--' pages and look for EULA agreement prompt
            for _ in range(50):  # max 50 space presses to avoid infinite loop
                if '--More--' in out:
                    self.write_raw(' ')  # Send space
                    time.sleep(0.5)
                    out = self.read()
