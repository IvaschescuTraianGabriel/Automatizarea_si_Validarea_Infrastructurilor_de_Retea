...
from ftd_connection import ConfigurationFTD
...
class SetupTelnetConnections(aetest.CommonSetup):
    """
    Initialize and connect to devices via Telnet
    """
...
class ConnectingFTD(aetest.Testcase):
    ...
    def setup(self):
        ...
        for dev_name, dev in testbed.devices.items():
            ...
            if not conn_info or 'class' not in conn_info:
                log.warning(f"[SKIP] Device '{dev_name}' has no REST connection.")
                continue
    ...
    @aetest.test
    def run_ftd_configuration(self, steps: Steps):
        """Run configuration logic for each FTD"""
        for dev_name, swagger in self.swagger_objects.items():
            dev = testbed.devices[dev_name]
            log.info(f"[INFO] Running FTD configuration for '{dev_name}'...")
            try:
                config_ftd_device = ConfigurationFTD(swagger, dev)
                config_ftd_device.configure(steps) # apply the FTD configuration
            except Exception as e:
                log.error(f"[FAILED] Configuration for '{dev_name}' failed: {e}")
                self.failed(f"Configuration error on '{dev_name}': {e}")
